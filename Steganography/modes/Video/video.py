...
def decrypt(video):
    ...
    # Debugging: Check if frames exist
    frame_files = os.listdir(root)
    print("[DEBUG] Extracted frames:", frame_files)

    if not frame_files:
        print("[ERROR] No frames extracted!")
        return "Decryption failed: No frames found."
    ...
    for i in range(len(frame_files)):
        ...
        try:
            ...
            if secret_dec is None:
                print(f"[INFO] No hidden data in {f_name}, stopping...")
                break
            print(f"[INFO] Extracted from {f_name}: {secret_dec}")  # Debugging
            secret.append(secret_dec)
        except Exception as e:
            print(f"[ERROR] Failed to decode {f_name}: {e}")

    result = ''.join(secret)
    print("[INFO] Final extracted message:", result)
